...
### 11.7 面向 Owner 的双语写法（唯一真源）

**给 Owner 看的任何输出里，协议代号一律写成「中文 + 英文」或「代号＝中文含义」，不得裸写代号。** 本表是标准译法，各文件引用此处，不各写一套。
...
| Route | **路由 Route** |
| State | **学习状态 State** |
| Methods | **教学方法 Methods** |
| Oracle | **验收标准 Oracle** |
| Review | **复习探针 Review** |

**主合同取值（必须附中文全称，不得只写字母）**

| 代号 | 展示写法 |
|---|---|
| `M` | `M`＝心智模型与记忆（回忆、解释、辨析、长期保持） |
| `B` | `B`＝构建与行为（执行、构建、调试、真实产物） |
| `J` | `J`＝判断、探究与综合（研究、证据综合、概率判断、证伪与决策） |

**证据等级**

| 代号 | 展示写法 |
|---|---|
| `E0` | `E0` 只是接触过（AI 生成或照抄，不算会） |
| `E1` | `E1` 提示后完成（辅助学习，不算掌握） |
| `E2` | `E2` 本轮独立完成（暂定，需延迟或迁移复测） |
| `E3` | `E3` 延迟后独立复现或陌生迁移（已验证） |
| `E4` | `E4` 在真实产物或已解析判断中用出来（实战掌握） |
...
| 代号 | 展示写法 |
|---|---|
| `R0`–`R6` | 回忆强度等级（`R0` 最弱，`R6` 最稳） |
| `active` / `paused` | 在跑 `active` / 暂停 `paused` |
| `merged` / `archived` | 已并入他项 `merged` / 归档 `archived` |
| `unknown` | 证据冲突 `unknown` |
| `current` / `recheck_due` / `superseded` | 当前有效 `current` / 需复查 `recheck_due` / 已被取代 `superseded` |

内部文件之间（治理文件正文、机器可读状态、脚本）**可以**只写代号；**只有面向 Owner 的教学输出与报告强制双语**。
