...
| `v0.0.0.8` | 2026-07-22 | 新增 `_system/templates/REFERENCE_LESSON_SKELETON.md`：把 `v0.0.0.7` 的达标线表变成**看得见的密度基准** —— 四段八槽，每槽一段合格示范 + 一段不合格反例。原因是**规则只能规定写什么，规定不了写多密**，而大模型照抄样例远比照抄规则准。示例主题刻意取中性载体（成本换概率收益的阈值判断），不绑任何真实项目；槽位 7 现实迁移按定义必须绑真实项目，故只给形状不给内容 |
| `v0.0.0.7` | 2026-07-22 | Owner 第三次反馈「太浅太单薄」。**前两次只改措辞均无效，本次改机制**：八项深度各自增加最小合格标准（唯一真源在 `TEACHING_METHOD_REGISTRY.md` 文末），低于线即未交付；**删掉两个逃生舱** —— 「不按字数机械衡量」与模型可自判的「本次时间极短」；新增四段骨架与交付前静默自检；**不设课程档位**，默认永远最深 |
| `v0.0.0.6` | 2026-07-22 | Owner 授权：**协议代号一律中英双语**，不得裸写 `Route=J`、`E2`、`H1`、`active` 等；标准译法集中在口径字典 11.7，适用于整条面向 Owner 的输出 |
| `v0.0.0.5` | 2026-07-22 | Owner 授权：「今日课程状态」块改为**渲染后的 Markdown**（二级标题 + 加粗字段 + 引用块 + 五行表格），不再用代码围栏输出纯文本；时区改为跟随 Owner 当前所在地，不硬编码 |
