...
- 分支：`main`（**唯一分支**）
- 本次任务基线提交：`73ca791`（`v0.0.0.7` 可度量深度下限）
...
- 更新时间：`2026-07-22T13:20Z`
- 工作树：干净（实施隔离在工作树 `_scratch/notionstudy-reference-lesson` / 分支 `feat/reference-lesson-skeleton`，快进合并到 `main` 后工作树与分支已回收）
...
- 任务模式：`root_change`（Owner 明确授权：产出通用参考课骨架作为密度基准）
...
- 已完成：
  ...
  - `v0.0.0.7`：教学深度下限改成可度量、可失败的机制。Owner 已是第三次反馈「太浅太单薄」，前两次只改措辞均无效 —— 病根是八项深度清单**可以被八句话满足**，且规则里留了两个逃生舱。本次：① 八项各自增加最小合格标准（唯一真源在 `TEACHING_METHOD_REGISTRY.md` 文末），低于线即未交付；② 删掉「不按字数机械衡量」（删了唯一可度量的下限却没换上能失败的东西）和「本次时间极短」（**模型自己就能判定**，等于把钥匙交给被约束的一方）；③ 新增四段骨架与交付前静默自检；④ **不设课程档位**，默认永远最深，唯一压缩条件是 Owner 亲口说「太长」且只压当轮（提交 `73ca791`）
  - **`v0.0.0.8`（本次）：** 新增 `_system/templates/REFERENCE_LESSON_SKELETON.md`。`v0.0.0.7` 解决了「要写什么」，解决不了「要写多密」—— **规则规定不了密度，样例可以**。四段八槽，每槽一条达标线 + 一段合格示范 + 一段不合格反例（**反例是这份文件的全部价值**，只给正面示范学不到「什么算没写够」）。Owner 明确要求**不绑真实项目**，故示例主题取中性载体（成本换概率收益的阈值判断）；槽位 7「现实迁移」按定义必须绑真实项目，因此**唯一没有正面示范的槽位**，只给形状
- 失败或跳过：无
...
- 命令：
  - `bash _system/validation/conformance_check.sh` —— 136/136 通过，退出码 0（新增 `G28` 参考课骨架门）
...
- **长期不变量（每次写入后必须仍成立）：**
  ...
  8. 参考课骨架存在、四段齐全、八槽各有达标线与**不合格反例**、自检表八行、五个文件都指向它、示例主题声明为项目中性（`G28` 断言，已用五次注入测试验证非恒真门：删掉任一槽的反例、整段删掉一个槽、抽掉某个文件的指针、删掉「与真实项目无关」声明、自检表少一行 —— 五次均被拦下）。
...
8. **`v0.0.0.7` 与 `v0.0.0.8` 需要 Owner 再粘一次**：把 `_system/templates/CHATGPT_PROJECT_INSTRUCTIONS.md` 里「———」之间的内容重新粘进 ChatGPT 的 Project Instructions。仓库改了**不会**自动改 ChatGPT 侧 —— 不重粘，它会继续按旧的八项清单讲浅课，也不会去读参考课骨架。
9. **深度是否真的上去了，只能由下一次真实上课来验收。** `G27` / `G28` 只能保证规则与样例本身不被稀释，**无法检查一节课的实际质量** —— 教学输出不经过仓库，四道门看不见它。下一次上课后由 Owner 判定：仍然浅 → 病根不在规则文本也不在样例，应转向别的机制（Owner 已明确暂不采用「一课拆多轮」，那是下一个候选）。
10. `G28` 能验骨架的**结构**（槽位在不在、反例在不在），**验不了达标线本身的质量** —— 把某条达标线改写成一句空话仍能过门。后继 Agent 修改该文件时必须人工判读，不能只看退出码。
11. 四道门的中文门解析器此前有缺陷：正文里用四反引号包三反引号会打乱围栏配对，导致大量正文被误当未登记术语。已按反引号数量精确配对修复，并用注入测试确认修复后仍能抓到真正未登记的术语（不是把门改松了）。
...
- 已知风险：
  ...
  - **命令行默认工作目录是主工作树，不是工作树。** 本次曾用相对路径跑 `sed`，改动静默落进主树 `NotionStudyProject/`（违反铁律 2）；因为改动全是版本号、无独有内容，已 `git checkout -- .` 完整还原并在工作树重做，主树复核为 0 脏文件。**后继 Agent 在工作树作业前先 `cd` 进工作树或一律用绝对路径**，别假设 `git worktree add` 会切换当前目录。
...
1. 直接开始学习，例如 `开始学习：AI 工作流操作系统，直接进第 6 课的严重失败判定` —— **这同时是 `v0.0.0.7` + `v0.0.0.8` 的实测**：课讲完后由 Owner 判定深度是否真的上去了；
...
# 只撤销 v0.0.0.8 骨架变更，保留 v0.0.0.7
git revert --no-edit 73ca791..HEAD
# 全量回到安装前快照（仅在 Owner 明确要求时）
git reset --hard study-os-v0.0.0.2-pre-20260721T225052Z
```

- 恢复被重写的任一治理文件：`git show 73ca791:<文件名>`
