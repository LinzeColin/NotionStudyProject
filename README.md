...
教学方的启动核验（读仓库、解析项目、选路由、加载状态）**默认静默执行，不展示**。你看到的回复直接以「今日课程状态」开头 —— 一个**渲染后的 Markdown 块**（二级标题 + 加粗字段 + 引用块 + 五行表格），含日期、课程导航、主题、项目状态与已验收进度，以及路由 Route / 学习状态 State / 教学方法 Methods / 验收标准 Oracle / 复习探针 Review，随后直接进正课。

**协议代号一律中英双语**：不会出现让人看不懂的 `Route=J`，而是写成「路由 Route：`J`＝判断、探究与综合」。标准译法见 [`STUDY_ARCHITECTURE.md`](STUDY_ARCHITECTURE.md) 11.7。模板见 [`SESSION_ACCEPTANCE_AND_REVIEW.md`](SESSION_ACCEPTANCE_AND_REVIEW.md) 1.2。
